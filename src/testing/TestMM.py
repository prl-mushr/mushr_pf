...
def test_mm_1(motion_model,counts):
    ...
    counts.append(model_error(motion_model.particles,[0.09937757, 0.0033936,  0.10634897]))
    return counts

def test_mm_2(motion_model,counts):
    motion_model.apply_motion_model(motion_model.particles,[3.0,0.4,0.5])
    counts.append(model_error(motion_model.particles,[0.76003705, 0.99738116, 1.86917909]))
    return counts

def test_mm_3(motion_model,counts):
    motion_model.apply_motion_model(motion_model.particles,[3.0,0.4,0.5])
    counts.append(model_error(motion_model.particles,[0.7661687,  0.97692261, 1.85254576]))
    return counts
...
if __name__ == '__main__':
    ...
    plt.quiver(motion_model.particles[:,0], motion_model.particles[:,1],arrow_thickness*np.cos(motion_model.particles[:,2]),arrow_thickness*np.sin(motion_model.particles[:,2]), color='b')
    plt.show()
