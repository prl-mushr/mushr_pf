...
def callback(true_pose,pf_pose):
    ...
    error_t = (Utils.quaternion_to_angle(true_pose.pose.orientation) - Utils.quaternion_to_angle(pf_pose.pose.orientation))**2
    error_array.append([error_x,error_y,error_t])
    print_array = np.array(error_array)
    print "Median X Error:",np.median(print_array[:,0]),"Median Y Error:",np.median(print_array[:,1]),"Median Theta Error:",np.median(print_array[:,2])
